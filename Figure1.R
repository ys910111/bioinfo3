...
names(maf_kras_hotspot)[1:2] = c('sample', 'Hotspot')
# kras_vaf = data.frame(maf_kras@data %>% summarise(`sample`= `Tumor_Sample_Barcode`, `KRAS VAF`=t_alt_count/t_depth) %>% group_by(`sample`) %>% filter(`KRAS VAF` == max(`KRAS VAF`)), check.names = F)
kras_vaf = data.frame(maf_kras_hotspot %>% summarise(`sample` = `sample`, `KRAS VAF`=t_alt_count/t_depth, `Hotspot` = `Hotspot`), check.names = F)
...
molecular = read.xlsx('../Table S1.xlsx', 3)
...
thres = fread('../574364/all_thresholded.by_genes.txt')
apply(thres[,-(1:3)], 2, function(x){sum(na.omit(x) > 0)}) %>% max
apply(thres[,-(1:3)], 2, function(x){-sum(na.omit(x) < 0)}) %>% min
