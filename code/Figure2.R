...
samples_100 = as.character(anno_df$sample[anno_df$`Tumor cellularity` == ">=15%, Sufficient Purity (n = 105)" & anno_df$`Histological type` == 'PDAC'])
...
sites =c("CREBBP-S274", "E2F4-S244", "FOXO1-S256", "FOXO1-S298", "HDAC1-S406", "HDAC1-S410", "HDAC1-S421", "JUNB-T255", "NCOR1-S1981", "NPM1-S214","NPM1-S70", 'SMARCA4-S613', 'SMARCC2-S367', 'TRIM33-S1088', 'ZFP36-S119', 'ZFP36-S66')

cnv_gene_100 = cnv_gene_105[, samples_100]
rna_100 = rna_105[, samples_100]
prot_100 = prot_105[, samples_100]


group_wilcox = function(cnv, genes, cnv_df, rna_df, prot_df){
  x = data.frame(apply(cnv_df[cnv,], 1, function(x){factor(ifelse(x < -0.4, 'CNV', 'Ctrl'))}), sample = names(cnv_df))
  y = data.frame(t(rna_df[intersect(genes, rownames(rna_df)),]), sample = names(rna_df))
  z = data.frame(t(prot_df[intersect(genes, rownames(prot_df)),]), sample = names(prot_df))
  merged = Reduce(function(x, y){merge(x, y, by = 'sample')}, list(x, y, z))
  names(merged)[-1] = c(paste0(cnv, '_CNV'), paste0(intersect(genes, rownames(rna_df)), '_RNA'), paste0(intersect(genes, rownames(prot_df)), '_Protein'))
  results_df = data.frame()
  for (c in cnv){
    for (g in genes){
      for (t in c('RNA', 'Protein')){
        try({
          if (all(table(merged[!is.na(merged[,str_glue('{g}_{t}')]),str_glue('{c}_CNV')]) > 3)){
            p = wilcox.test(as.formula(str_glue('{g}_{t} ~ {c}_CNV')), merged)$p.value
            fc = median(na.omit(merged[merged[,str_glue('{c}_CNV')] == 'CNV', str_glue('{g}_{t}')])) - median(na.omit(merged[merged[,str_glue('{c}_CNV')] == 'Ctrl', str_glue('{g}_{t}')]))
            results_df = rbind(results_df, data.frame(c, g, t, fc, p))
          } 
        })
      }
    }
  }
  names(results_df) = c('source', 'target', 'type', 'fc', 'P')
  return(results_df)
}

group_wilcox_phos = function(cnv, sites, cnv_df, phos_df){
  x = data.frame(apply(cnv_df[cnv,], 1, function(x){factor(ifelse(x < -0.4, 'CNV', 'Ctrl'))}), sample = names(cnv_df))
  y = data.frame(t(phos_df[intersect(sites, rownames(phos_df)),]), sample = names(phos_df))
  merged = Reduce(function(x, y){merge(x, y, by = 'sample')}, list(x, y))
  names(merged)[-1] = c(paste0(cnv, '_CNV'), paste0(intersect(sites, rownames(phos_df)), '_Phos'))
  results_df = data.frame()
  for (c in cnv){
    for (g in sites){
      try({
        if (all(table(merged[!is.na(merged[,str_glue('{g}_Phos')]),str_glue('{c}_CNV')]) > 3)){
          p = wilcox.test(as.formula(str_glue('`{g}_Phos` ~ {c}_CNV')), merged)$p.value
          fc = median(na.omit(merged[merged[,str_glue('{c}_CNV')] == 'CNV', str_glue('{g}_Phos')])) - median(na.omit(merged[merged[,str_glue('{c}_CNV')] == 'Ctrl', str_glue('{g}_Phos')]))
          results_df = rbind(results_df, data.frame(c, g, fc, p))
        } 
      })
    }
  }
  names(results_df) = c('source', 'target', 'fc', 'P')
  return(results_df)
}

results = group_wilcox(cnv, genes, cnv_gene_105, rna_105, prot_105)
results_phos = group_wilcox_phos(cnv, sites, cnv_gene_105, phos_105)


get_Fig2H = function(source, target, cnv_df, rna_df, prot_df) {
  Mutation_gene_sel = data.frame(t(cnv_df[source,]), case_id = colnames(cnv_df))
  Mutation_gene_sel$deletion = ifelse(Mutation_gene_sel[,source] < -0.4, 'Del', 'Control')
  names(Mutation_gene_sel)[1] = 'cnv'
  print(source)
  
  #RNA
  if (!target %in% rownames(rna_df)) {
    rna_change = NA
    rna_P = NA
  }else {
    RNA_gene_sel = data.frame(t(rna_df[target,]), case_id = colnames(rna_df))
    print(paste("RNA", target))
    allD = merge(Mutation_gene_sel, RNA_gene_sel, by = "case_id")
    if (sum(!is.na(allD[allD$deletion=="Control", target])) < 3 | sum(!is.na(allD[allD$deletion=="Del", target])) < 3) {
      rna_change=NA
      rna_P=NA
    }else {
      rna_change = median(na.omit(allD[allD$deletion=="Del", target])) - median(na.omit(allD[allD$deletion=="Control", target]))
      rna_P  = wilcox.test(allD[allD$deletion=="Del", target], allD[allD$deletion=="Control", target])[["p.value"]]
    }
  }
  
  #Protein
  if (!target %in% rownames(prot_df)) {
    prot_change = NA
    prot_P = NA
  }else {
    PROT_gene_sel = data.frame(t(prot_df[target,]), case_id = colnames(prot_df))
    print(paste("PROT", target))
    allD = merge(Mutation_gene_sel, PROT_gene_sel, by = "case_id")
    if (sum(!is.na(allD[allD$deletion=="Control", target])) < 3 | sum(!is.na(allD[allD$deletion=="Del", target])) < 3) {
      prot_change=NA
      prot_P=NA
    }else {
      prot_change = median(na.omit(allD[allD$deletion=="Del", target])) - median(na.omit(allD[allD$deletion=="Control", target]))
      prot_P = wilcox.test(allD[allD$deletion=="Del", target], allD[allD$deletion=="Control", target])[["p.value"]]
    }
  }
  
  result = data.frame(source = source, 
                      target = target, 
                      target_t = c("RNA", "Protein"),
                      fchange = c(rna_change, prot_change), 
                      P = c(rna_P, prot_P), 
                      stringsAsFactors = F)
  return(result)
}

allresult = lapply(1:length(s_gene), function(x) get_Fig2H(s_gene[x], t_gene[x], cnv_gene_105, rna_105, prot_105))%>%rbindlist() 
allresult = subset(allresult, P < 0.05)

results_sub = subset(results, P < 0.02)
ggplot(results_sub, aes(x = target, y = paste0(source, '_', type), fill = fc, size = -log10(P), shape = ifelse(source == target, 'Cis', 'Trans'))) +
  geom_point() +
  scale_fill_gradient2(low = "blue", mid = 'white', high = "red", limits = c(-3, 3)) +
...
phos_105 = data.frame(subset(phos, select = samples), check.names = F)
rn = paste0(phos$Gene, '-', sapply(strsplit(phos$Index, '_'), function(x){x[3]}))
phos_105 = phos_105[!duplicated(rn),]
rownames(phos_105) = rn[!duplicated(rn)]

get_Fig2I = function(source, target) {
  Mutation_gene_sel = data.frame(t(cnv_gene_105[source,]), case_id = colnames(cnv_gene_105))
  Mutation_gene_sel$deletion = ifelse(Mutation_gene_sel[,source] < -0.4, 'Del', 'Control')
  names(Mutation_gene_sel)[1] = 'cnv'
  print(source)
  
  #Phosphosite
  if (!target %in% rownames(phos_105)) {
    phos_change = NA
    phos_P = NA
  }else {
    Phos_gene_sel =  phos_105%>%filter(rownames(.) == target)%>%
      t()%>%as.data.frame()%>%
      mutate(case_id = rownames(.))
    print(paste("Phos", target))
    allD = full_join(Mutation_gene_sel, Phos_gene_sel, by = "case_id")%>%
      filter(!is.na(target))
    if (sum(!is.na(allD[allD$deletion=="Control", target]))<3 | sum(!is.na(allD[allD$deletion=="Del", target]))<3) {
      phos_change=NA
      phos_P=NA
    }else {
      phos_change = median(na.omit(allD[allD$deletion=="Del", target])) - median(na.omit(allD[allD$deletion=="Control", target]))
      phos_P  = wilcox.test(allD[allD$deletion=="Del", target], allD[allD$deletion=="Control", target])[["p.value"]]
    }
  }
  result = data.frame(source = source, 
                      target = target, 
                      target_t = c("Phos"),
                      fchange = c(phos_change), 
                      P = c(phos_P), 
                      stringsAsFactors = F)
  result
}

s_gene=c(rep("SMAD4", length(t_gene)/2), rep("CDKN2A", length(t_gene)/2))

allresult = lapply(1:length(s_gene), function(x) get_Fig2I(s_gene[x], t_gene[x]))%>%rbindlist()
allresult = subset(allresult, P < 0.05)

results_sub = subset(results_phos, P < 0.03)
ggplot(results_sub, aes(x = target, y = source, fill = fc, size = -log10(P), shape = ifelse(source == target, 'Cis', 'Trans'))) +
  geom_point() +
